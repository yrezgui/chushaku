...
gulp.task('scripts', function scriptsTask() {
  return es.concat(
    ...
    gulp.src(config.vendor.scripts).pipe(changeFolder('deps')).pipe(updateAssetsMap('dependencies'))
...
gulp.task('styles', function stylesTask() {
  return es.concat(
    gulp.src(config.path.styles).pipe(stylus({errors: true})),
...
function processViews() {
  return gulp.src(config.path.views)
             .pipe(jade({pretty: true, locals: {config: config, assets: assetsMap}}))
             .pipe(gulp.dest(config.path.build));
}
